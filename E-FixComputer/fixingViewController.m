...
- (void)viewDidLoad{
    ...
    if ([publicMethod Add_appDelegate].loginuser != nil) {
        [publicMethod AsynchronousGainData:self.request Target:self Path:[NSString stringWithFormat:@"order/UserWaitOrder?user_id=%@&sendstatus=1",[publicMethod Add_appDelegate].loginuser.userid]];
    }
    if ([publicMethod Add_appDelegate].loginengineer != nil){
        [publicMethod AsynchronousGainData:self.request Target:self Path:[NSString stringWithFormat:@"order/engineerWaitAcceptorder?receiver_id=%@&receivestatus=1&sendstatus=1",[publicMethod Add_appDelegate].loginengineer.engineerid]];
    }
...
- (UITableViewCell*)getSendList:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if ([self.SendOrderList count]) {
        ...
        if ([order.receivestatus isEqualToString:@"1"]) {
            cell.lablestatus.text = @"被接受";
        }else{
            cell.lablestatus.text = @"被拒绝";
        }
        ...
        if (sign) {
            ...
            cell.textLabel.text = @"您没有被接受或拒绝预约";
            sign = YES;
        }
        return cell;
    }
